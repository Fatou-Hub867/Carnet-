...
async def create_availability(
    db: AsyncSession, doctor_id: int, data: AvailabilityCreateRequest
) -> Availability:
    if data.end_time <= data.start_time:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "end_time must be after start_time"
        )
    if data.date < date.today():
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "Cannot open a slot in the past"
        )
    ...
    if overlap is not None:
        raise HTTPException(
            status.HTTP_409_CONFLICT, "This slot overlaps an existing availability"
        )
...
async def list_doctor_availabilities(
    db: AsyncSession, doctor_id: int, from_date: date
) -> list[Availability]:
...
async def _get_owned_pending_appointment(
    db: AsyncSession, doctor_id: int, appointment_id: int
) -> Appointment:
    ...
    if appointment.status != AppointmentStatus.PENDING:
        raise HTTPException(
            status.HTTP_409_CONFLICT, "Only a pending appointment can be decided"
        )
    return appointment
...
async def complete_appointment(
    ...
    if appointment.status != AppointmentStatus.CONFIRMED:
        raise HTTPException(
            status.HTTP_409_CONFLICT, "Only a confirmed appointment can be completed"
        )
