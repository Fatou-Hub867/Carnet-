...
- **Doctor availability toggle**: `Doctor.is_available` is enforced server-side, not just a UI hint — booking a slot from an unavailable doctor is rejected with 409 (`features/Appointments/logic.book_appointment`).
- **Password change vs. reset**: logged-in patients/doctors change their password via `POST /auth/*/change-password` (current password required); the forgot-password email flow is separate and only for logged-out users.
